...
std::optional<SystemMemoryInfo> GetSystemMemoryInfo() {
  ...
  if (sysctl(mib, 2, &physical_memory, &length, nullptr, 0) != 0) {
    std::cerr << "Failed to get total physical memory (sysctl)" << std::endl;
    return std::nullopt;
  }
  ...
  if (info.total_physical_bytes == 0) {
    std::cerr << "Failed to parse total physical memory from /proc/meminfo" << std::endl;
    return std::nullopt;
  }

#else
  std::cerr << "Unsupported platform for memory info" << std::endl;
  return std::nullopt;
#endif
...
std::optional<ProcessMemoryInfo> GetProcessMemoryInfo() {
  ...
  if (info.rss_bytes == 0) {
    std::cerr << "Failed to parse RSS from /proc/self/status" << std::endl;
    return std::nullopt;
  }

#else
  std::cerr << "Unsupported platform for process memory info" << std::endl;
  return std::nullopt;
#endif
...
bool CheckMemoryAvailability(uint64_t required_bytes, double safety_margin_ratio) {
  ...
  if (!system_info) {
    std::cerr << "Unable to check memory availability, allowing operation" << std::endl;
